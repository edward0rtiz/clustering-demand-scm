...
conditions = [(((dforder['client_lat'] == dforder['restaurant_lat']) & (
            dforder['client_lng'] == dforder['restaurant_lng']) & (dforder['accepted_by_restaurant'].isna()))),
              (((dforder['client_lat'] != dforder['restaurant_lat']) & (dforder['client_lng']
                                                                        != dforder['restaurant_lng']) & (
...
kiwers_dayxw = dfkiwer.groupby(by=['weekday', 'week'], as_index=False)['username'].count()
...
drop_cols = ['client_id', 'client_lat', 'client_lng', 'client_readable_address',
...
categorical_dimensions = ['type_of_delivery', 'client_is_prime', 'payment_method', 'order_status']

dimensions = [dict(values=dforder_bot[label], label=label) for label in categorical_dimensions]
...
parallel = go.FigureWidget(
    data=[go.Scatter(x=dforder_bot.bot_name, y=dforder_bot['price_without_discounts'],
...
parallel.update_layout(
    height=800, xaxis={'title': 'bot_name'},
...
parallel.update_layout({'plot_bgcolor': 'rgba(0, 0, 0, 0)',
                        'paper_bgcolor': 'rgba(0, 0, 0, 0)',
                        })
parallel.update_xaxes(showgrid=True, gridwidth=1, gridcolor='LightGrey',
                      zeroline=True, zerolinewidth=2, zerolinecolor='LightGrey')
parallel.update_yaxes(showgrid=True, gridwidth=1, gridcolor='LightGrey',
                      zeroline=True, zerolinewidth=2, zerolinecolor='LightGrey')

################## [Plot 2/17 Heatmap] Correlation of cols ######################
dfcorr = dforder[
    ['price_without_discounts', 'price_with_discounts', 'delivey_fee', 'order_tax', 'order_tip', 'total_cost']].corr()
dfcorr_list = []
for i in list(dfcorr):
    dfcorr_list.append(dfcorr[i].round(2).tolist())
x_list = ['price_without_discounts', 'price_with_discounts', 'delivey_fee', 'order_tax', 'order_tip', 'total_cost']
y_list = ['price_without_discounts', 'price_with_discounts', 'delivey_fee', 'order_tax', 'order_tip', 'total_cost']
heatmap_1 = ff.create_annotated_heatmap(z=dfcorr_list, x=x_list, y=y_list, annotation_text=dfcorr_list,
                                        colorscale='Viridis')
heatmap_1.layout.width = 600

################# [Plot 3/17 Heatmap] Correlation of cols ########################
dfcorr2 = dforder[['client_is_prime', 'price_without_discounts', 'price_with_discounts',
...
y_list2 = ['client_is_prime', 'price_without_discounts', 'price_with_discounts',
           ...
           'wait_battery', 'total_cost', 'diff_closed',
           'hour', 'month']
heatmap_2 = ff.create_annotated_heatmap(z=dfcorr_list2, x=x_list2, y=y_list2, annotation_text=dfcorr_list2,
                                        colorscale='Viridis')
...
dfcust = dforder[['created', 'client_id', 'client_is_prime', 'week_no']].sort_values(by=['client_id', 'created'])
...
df_type_client = pd.Series(cust_dict).reset_index().rename(columns={'index': 'Type of Client', 0: 'Number of Clients'})
df_type_client = df_type_client.assign(Client=['Client', 'Client', 'Client'])

type_plot = px.bar(df_type_client, x='Client', y='Number of Clients', color='Type of Client', title="Type of Client",
                   height=400)
type_plot.update_layout({'plot_bgcolor': 'rgba(0, 0, 0, 0)',
                         'paper_bgcolor': 'rgba(0, 0, 0, 0)',
                         })
type_plot.update_xaxes(showgrid=True, gridwidth=1, gridcolor='LightGrey',
                       zeroline=True, zerolinewidth=2, zerolinecolor='LightGrey')
type_plot.update_yaxes(showgrid=True, gridwidth=1, gridcolor='LightGrey',
                       zeroline=True, zerolinewidth=2, zerolinecolor='LightGrey')
...
dfcusttable = dfcusttable.groupby(['week_no']).agg({'Prime': ['sum'], 'No Prime': ['sum']})
dfcusttable.reset_index(inplace=True)
dfcusttable.columns = dfcusttable.columns.droplevel(1)
client_count_plot = go.Figure(data=[
...
client_count_plot.update_layout(barmode='stack', title_text="Count type of client by week")
client_count_plot.update_layout({'plot_bgcolor': 'rgba(0, 0, 0, 0)',
                                 'paper_bgcolor': 'rgba(0, 0, 0, 0)',
                                 })
client_count_plot.update_xaxes(showgrid=True, gridwidth=1, gridcolor='LightGrey',
                               zeroline=True, zerolinewidth=2, zerolinecolor='LightGrey')
client_count_plot.update_yaxes(showgrid=True, gridwidth=1, gridcolor='LightGrey',
                               zeroline=True, zerolinewidth=2, zerolinecolor='LightGrey')

############################# [6/17 Plot] Orders by date range ###################################
dforder_dates = dforder.copy()
dforder_dates['creation_date'] = dforder['created'].dt.date
dforder_dates = dforder_dates.groupby('creation_date', sort=True).sum().rename(columns={'client_is_prime': 'orders'})
dforder_dates.index = pd.to_datetime(dforder_dates.index)

############################# [7/17 Plot] Orders by week ###################################
dforder_weeks = dforder.groupby('week_no').sum().rename(columns={'client_is_prime':'orders'})
...
dforder_days = dforder['weekday'].value_counts(sort=False).rename_axis('Weekday').\
...
dforder_xm = dforder.groupby(by=['weekday', 'month'], as_index=False)['order_id'].count()


############# [11/17 Plot] Evolution of Hour Monthly-Orders by hours ################
############## [12/17 Plot] Evolution of each hour by month ###############
dforder_hourxm = dforder.groupby(by=['hour', 'month'], as_index=False)['order_id'].count()

########################[13/17 Plot] Boxplots: take data from dforder ######################
boxplot_cost = make_subplots(rows=1, cols=2, subplot_titles=("By payment method", "By Client is prime"))
boxplot_cost.add_trace(
    go.Box(x=dforder["payment_method"], y=dforder["total_cost"]),
    row=1, col=1
)
boxplot_cost.add_trace(
    go.Box(x=dforder["client_is_prime"], y=dforder["total_cost"]),
    row=1, col=2
)
boxplot_cost.update_layout(title_text="Total Cost (USD) by Payment method and Type of client")
boxplot_cost.update_layout({'plot_bgcolor': 'rgba(0, 0, 0, 0)',
                            'paper_bgcolor': 'rgba(0, 0, 0, 0)'})
boxplot_cost.update_xaxes(showgrid=True, gridwidth=1, gridcolor='LightGrey',
                          zeroline=True, zerolinewidth=2, zerolinecolor='LightGrey')
boxplot_cost.update_yaxes(showgrid=True, gridwidth=1, gridcolor='LightGrey',
                          zeroline=True, zerolinewidth=2, zerolinecolor='LightGrey')

################################[14/17 Plot] Total cost ###################################
dforder_totalcostxw = dforder.groupby('week_no').sum().reset_index()
totalcost_byweek = px.bar(dforder_totalcostxw, x="week_no", y="total_cost", title='Total cost by weeks')
totalcost_byweek.update_layout({'plot_bgcolor': 'rgba(0, 0, 0, 0)',
                                'paper_bgcolor': 'rgba(0, 0, 0, 0)'})
totalcost_byweek.update_xaxes(showgrid=True, gridwidth=1, gridcolor='LightGrey',
                              zeroline=True, zerolinewidth=2, zerolinecolor='LightGrey')
totalcost_byweek.update_yaxes(showgrid=True, gridwidth=1, gridcolor='LightGrey',
                              zeroline=True, zerolinewidth=2, zerolinecolor='LightGrey')

############################### [15/17 Plot] Total tips by hour ##############################
dforder_tipsxh = dforder[dforder['order_tip'] > 0].groupby('hour').sum().reset_index()
tips_hour = px.bar(dforder_tipsxh, x='hour', y='order_tip', title='Total tips by hour',
                   labels={'hour': 'Hour', 'order_tip': 'Tips (USD)'})
tips_hour.update_layout({'plot_bgcolor': 'rgba(0, 0, 0, 0)',
                         'paper_bgcolor': 'rgba(0, 0, 0, 0)'})
tips_hour.update_xaxes(showgrid=True, gridwidth=1, gridcolor='LightGrey',
                       zeroline=True, zerolinewidth=2, zerolinecolor='LightGrey')
tips_hour.update_yaxes(showgrid=True, gridwidth=1, gridcolor='LightGrey',
                       zeroline=True, zerolinewidth=2, zerolinecolor='LightGrey')
...
dforder_busiest_day = dforder[(dforder['type_of_delivery'] == 'bot') & (dforder['created'].dt.date == busiest_day)]
dforder_busiest_day = dforder_busiest_day.groupby(['hour'])['bot_name'].unique().str.len().reset_index()
bots_busiest_day = px.bar(dforder_busiest_day, x="hour", y="bot_name", title='Bots demand by hour on busiest day',
                          labels={"hour": "Hour", "bot_name": "Number of bots"})
bots_busiest_day.update_layout({'plot_bgcolor': 'rgba(0, 0, 0, 0)',
                                  'paper_bgcolor': 'rgba(0, 0, 0, 0)'})
bots_busiest_day.update_xaxes(showgrid=True, gridwidth=1, gridcolor='LightGrey',
                                zeroline=True, zerolinewidth=2, zerolinecolor='LightGrey')
bots_busiest_day.update_yaxes(showgrid=True, gridwidth=1, gridcolor='LightGrey',
                                zeroline=True, zerolinewidth=2, zerolinecolor='LightGrey')

############################  [17/17 Plot] Number of orders made by prime client - Price ##################
dforder_res = dforder.groupby('restaurant_name').sum().reset_index()
restaurant_summary = px.scatter(dforder_res,
                                x="client_is_prime",
                                y="restaurant_name",
...
restaurant_summary.update_layout({'plot_bgcolor': 'rgba(0, 0, 0, 0)',
                                  'paper_bgcolor': 'rgba(0, 0, 0, 0)'})
restaurant_summary.update_xaxes(showgrid=True, gridwidth=1, gridcolor='LightGrey',
                                zeroline=True, zerolinewidth=2, zerolinecolor='LightGrey')
restaurant_summary.update_yaxes(showgrid=True, gridwidth=1, gridcolor='LightGrey',
                                zeroline=True, zerolinewidth=2, zerolinecolor='LightGrey')
